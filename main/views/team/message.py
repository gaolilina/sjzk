...
class Contacts(View):
    ...
    def get(self, request, team, offset=0, limit=10):
        ...
        c = team.contacts.count()
        qs = team.contacts.all()[i:j]
        l = [{'id': r.contact.id,
              'username': r.contact.username,
              'icon_url': r.contact.icon_url,
              'unread_count': team.messages.filter(
...
class Messages(View):
    ...
    def get(self, request, team, user, offset=0, limit=10):
        ...
        i, j = offset, offset + limit
        qs = team.messages.filter(other_user=user)
        c = qs.count()
        ...
        team.messages.filter(id__in=ids).update(is_read=True)
    ...
    def post(self, request, team, user, content):
        ...
        msg = team.messages.create(other_user=user, direction=1,
                                           content=content, is_sharing=False)
        r, created = team.contacts.get_or_create(
