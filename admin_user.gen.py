...
import admin.models.admin_user as admin_users

url_text = """# Auto generated by admin_user.py
...
view_text = """# Auto generated by admin_user.py
...
view_class_text = """class {{cls_name}}View(View):
    ...
    def get(self, request, mod):
        template = loader.get_template("admin_user/{{tbl_name}}.html")
...
class {{cls_name}}List(View):
    ...
    def get(self, request, page=0, **kwargs):
        if kwargs["id"] is not None:
            list = {{cls_name}}.objects.filter(admin_user_id=kwargs["id"])
            template = loader.get_template("admin_user/{{tbl_name}}_list.html")
            context = Context({'page': page, 'list': list, 'redir': 'admin:admin_user:{{tbl_name}}', 'user': request.user})
            return HttpResponse(template.render(context))
        elif request.GET.get("name") is not None:
            name = request.GET.get("name")
            template = loader.get_template("admin_user/index.html")
            if {{cls_name}} == AdminUser:
                redir = 'admin:admin_user:admin_user'
            else:
                redir = 'admin:admin_user:{{tbl_name}}_list'
            context = Context({'name': name, 'list': AdminUser.objects.filter(name__contains=name), 'redir': redir, 'rb': '{{tbl_name}}', 'user': request.user})
            return HttpResponse(template.render(context))
        else:
            template = loader.get_template("admin_user/index.html")
...
for mod_name, mod_class in inspect.getmembers(admin_users):
    if mod_name in admin_users.__all__:
        template_text = """
{% extends "layout.html" %}
{% load staticfiles %}
{% block content %}
<form role="form" action="{% url 'admin:admin_user:{{tbl_name}}' mod.id %}" method="post">
<table>
    {{content}}
    <tr>
        <td>{{ msg }}</td>
...
{% block topbar %}{% include 'topbar.html' with tb='admin_user_admin' %}{% endblock %}
{% block leftbar %}{% include 'leftbar.html' with tb='admin_user_admin' lb='None' %}{% endblock %}
{% block rightbar %}{% include 'rightbar.html' with tb='admin_user_admin' lb='None' rb='{{tbl_name}}' %}{% endblock %}
...
"""
        ...
        for fld in mod_class._meta.get_fields():
            if isinstance(fld, models.CharField):
                ...
                content_text += '<tr><td>' + fld.name + '：</td><td><a href="{% url "admin:admin_user:' + fld.rel.to._meta.db_table + '" mod.' + fld.name + '.id %}">{{ mod.' + fld.name + '.id }}</a></td></tr>'
        
        view_text += view_class_text.replace('{{cls_name}}', mod_name).replace('{{tbl_name}}', tbl_name).replace('{{args}}', args_text)
        template_file = codecs.open("./admin/templates/admin_user/" + tbl_name + ".html", "w", "utf-8")
...
<table style="margin:30px">
    ...
    {% for item in list %}
        <tr>
            <td>用户名： </td>
            <td style="min-width: 200px;border:1px solid #C4D9AE;">{{ item.admin_user.name }}</td>
            <td><a href="{% url 'admin:admin_user:{{tbl_name}}' item.admin_user.id %}">查看</a></td>
        </tr>
    {% endfor %}
</table>
{% endblock %}
...
{% endblock %}
"""
        template2_file = codecs.open("./admin/templates/admin_user/" + tbl_name + "_list.html", "w", "utf-8")
        template2_file.write(template2_text.replace('{{tbl_name}}', tbl_name))
        template2_file.close()

url_text += "]"
url_file = codecs.open("./admin/urls/admin_user.py", "w", "utf-8")
url_file.write(url_text)
url_file.close()

view_file = codecs.open("./admin/views/admin_user.py", "w", "utf-8")
